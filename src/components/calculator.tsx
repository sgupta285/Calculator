...
const Calculator: React.FC = () => {
  ...
  // Operator buttons now use shadcn's default (primary) variant
  const operatorButtonClasses = ""; // No custom classes needed, rely on variant
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-80">
      ...
      <div className="grid grid-cols-4 gap-2">
        <Button className={`${buttonClasses} ${utilityButtonClasses} col-span-2`} onClick={handleClearClick}>C</Button>
        <Button className={`${buttonClasses} ${utilityButtonClasses}`} onClick={handleBackspaceClick}><Backspace className="h-6 w-6" /></Button>
        <Button className={`${buttonClasses} ${operatorButtonClasses}`} variant="default" onClick={() => handleOperatorClick('/')}>/</Button>
        ...
        <Button className={`${buttonClasses} ${operatorButtonClasses}`} variant="default" onClick={handleEqualsClick}>=</Button>
      </div>
    </div>
  );
};
